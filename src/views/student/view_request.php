...
<?php include "header.php"; ?>
    <div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <?php if (!empty($myRequest['existing_timetable_id'])): ?>
                <div>
                    <strong class="block text-gray-700 text-sm font-bold mb-2">Existing Timetable ID:</strong>
                    <p class="text-gray-900"><?php echo htmlspecialchars($myRequest['existing_timetable_id']); ?></p>
                </div>
            <?php endif; ?>

            <?php if ($myRequest['request_type'] === 'change_subject' && !empty($myRequest['proposed_subject_name'])): ?>
                <div>
            ...
            <?php if ($myRequest['request_type'] === 'change_teacher' && !empty($myRequest['proposed_teacher_name'])): ?>
                <div>
            ...
            <?php if ($myRequest['request_type'] === 'change_classroom' && !empty($myRequest['proposed_classroom_name'])): ?>
                <div>
            ...
            <div>
                <strong class="block text-gray-700 text-sm font-bold mb-2">Request Date:</strong>
                <p class="text-gray-900"><?php echo htmlspecialchars(date('F j, Y, g:i a', strtotime($myRequest['request_date']))); ?></p>
            </div>

            <?php if ($myRequest['requester_type'] === 'teacher' && !empty($myRequest['teacher_name_requested'])): ?>
                <div>
                    <strong class="block text-gray-700 text-sm font-bold mb-2">Requested By (Teacher):</strong>
                    <p class="text-blue-600 font-semibold"><?php echo htmlspecialchars($myRequest['teacher_name_requested']); ?></p>
                </div>
            <?php endif; ?>

            <?php if ($myRequest['requester_type'] === 'student' && !empty($myRequest['student_name_requested'])): ?>
                <div>
                    <strong class="block text-gray-700 text-sm font-bold mb-2">Requested By (Student):</strong>
                    <p class="text-blue-600 font-semibold"><?php echo htmlspecialchars($myRequest['student_name_requested']); ?></p>
                </div>
            <?php endif; ?>
            <div>
        ...
        <div class="mt-8">
            <a href="my_request.php" class="inline-block bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded">Back to Requests</a>
        </div>
    </div>
</body>
</html>